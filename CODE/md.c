...
// new vars
   ...
   for(i=1;i<=natoms1;i++) {
    ...
    x_2 = (double **) malloc((sizeof(double))*(natoms1+2));
    
    for(i=1;i<=natoms1;i++) {
      
       x_2[i] = (double *) malloc((sizeof(double))*(natoms1+2)); 
        
    };    
    y_2 = (double **) malloc((sizeof(double))*(natoms1+2));
    
    for(i=1;i<=natoms1;i++) {
      
       y_2[i] = (double *) malloc((sizeof(double))*(natoms1+2)); 
        
    };       
    z_2 = (double **) malloc((sizeof(double))*(natoms1+2));
    
    for(i=1;i<=natoms1;i++) {
      
       z_2[i] = (double *) malloc((sizeof(double))*(natoms1+2)); 
    ...
    snew(probability_x2,natoms1+1);
    snew(probability_y2,natoms1+1);
    snew(probability_z2,natoms1+1);
...
//                 dd_collect_state(cr->dd,state,state_global);
     ...
     if(stepper == 1) {       
       ...
       for(i=1;i<=natoms1;i++) {       
           ...
           probability_x2[i]    = 1.0;
           probability_y2[i]    = 1.0;
           probability_z2[i]    = 1.0;
...
for(i=1;i<=natoms1;i++){ 
        ...
        dL_x[i]     += atom_masses[i]*state_global->v[at_id[i]][0]*(state_global->x[at_id[i]][0] - x_1[i]);
        dL_y[i]     += atom_masses[i]*state_global->v[at_id[i]][1]*(state_global->x[at_id[i]][1] - y_1[i]);
        dL_z[i]     += atom_masses[i]*state_global->v[at_id[i]][2]*(state_global->x[at_id[i]][2] - z_1[i]);
        
        dLL_x[i]     = atom_masses[i]*state_global->v[at_id[i]][0]*(state_global->x[at_id[i]][0] - x_1[i]);
        dLL_y[i]     = atom_masses[i]*state_global->v[at_id[i]][1]*(state_global->x[at_id[i]][1] - y_1[i]);
        dLL_z[i]     = atom_masses[i]*state_global->v[at_id[i]][2]*(state_global->x[at_id[i]][2] - z_1[i]);
    ...
    if(diff_s_x1[i] - diff_s_x[i]/tau_1[i] != 0.0 && diff_s_x2[i] - diff_s_x[i]/tau_1[i] != 0.0) { 
        ...
        diff_L_xc[i] = (diff_s_x2[i] - diff_s_x[i]/tau_1[i])*(diff_s_x1[i] - diff_s_x[i]/tau_1[i])/
        (sqrt(pow(diff_s_x2[i] - diff_s_x[i]/tau_1[i],2))*sqrt(pow(diff_s_x1[i] - diff_s_x[i]/tau_1[i],2)));    
    ...
    if(diff_s_y1[i] - diff_s_y[i]/tau_1[i] != 0.0 && diff_s_y2[i] - diff_s_y[i]/tau_1[i] != 0.0) {
        ...
        diff_L_yc[i] = (diff_s_y2[i] - diff_s_y[i]/tau_1[i])*(diff_s_y1[i] - diff_s_y[i]/tau_1[i])/
        (sqrt(pow(diff_s_y2[i] - diff_s_y[i]/tau_1[i],2))*sqrt(pow(diff_s_y1[i] - diff_s_y[i]/tau_1[i],2)));
    ...
    if(diff_s_z1[i] - diff_s_z[i]/tau_1[i] != 0.0 && diff_s_z2[i] - diff_s_z[i]/tau_1[i] != 0.0) {
        ...
        diff_L_zc[i] = (diff_s_z2[i] - diff_s_z[i]/tau_1[i])*(diff_s_z1[i] - diff_s_z[i]/tau_1[i])/
        (sqrt(pow(diff_s_z2[i] - diff_s_z[i]/tau_1[i],2))*sqrt(pow(diff_s_z1[i] - diff_s_z[i]/tau_1[i],2)));
    ...
    if(ran <= 1.0/(1.0 + exp(-((diff_s_xc[i]/tau_1[i]))))) {
      
        diff_s_x2[i] = diff_s_x1[i];
        
    };
    
    if(ran <= 1.0/(1.0 + exp(-((diff_s_yc[i]/tau_1[i]))))) {    
        
        diff_s_y2[i] = diff_s_y1[i];        
        
    };
    
    if(ran <= 1.0/(1.0 + exp(-((diff_s_zc[i]/tau_1[i]))))) {    
        
        diff_s_z2[i] = diff_s_z1[i];
        
    }
    
    if(restraint_sampling[i] == 1) {
        
     dLL_x_r[i] = 0.0;
     dLL_y_r[i] = 0.0;     
     dLL_z_r[i] = 0.0;
     
     counter_restraint_x[i] = 0.0;
     counter_restraint_y[i] = 0.0;
     counter_restraint_z[i] = 0.0;
    ...
    for(k=1;k<=number_of_restraints;k++) {  
                        ...
                        if(i == at_id_r[k]) {
                           ...
                           dL_x_r[i]     += atom_masses[i]*state_global->v[at_id[i]][0]*(d_x - x_2[at_id_r[k]][at_id_r2[k]]);
                           dL_y_r[i]     += atom_masses[i]*state_global->v[at_id[i]][1]*(d_y - y_2[at_id_r[k]][at_id_r2[k]]);
                           dL_z_r[i]     += atom_masses[i]*state_global->v[at_id[i]][2]*(d_z - z_2[at_id_r[k]][at_id_r2[k]]);                           
                           
                           dLL_x[i]      += atom_masses[i]*state_global->v[at_id[i]][0]*(d_x - x_2[at_id_r[k]][at_id_r2[k]]);
                           dLL_y[i]      += atom_masses[i]*state_global->v[at_id[i]][1]*(d_y - y_2[at_id_r[k]][at_id_r2[k]]);
                           dLL_z[i]      += atom_masses[i]*state_global->v[at_id[i]][2]*(d_z - z_2[at_id_r[k]][at_id_r2[k]]);
                        ...
                        if(i == at_id_r2[k]) {
                           ...
                           dL_x_r[i]     += atom_masses[i]*state_global->v[at_id[i]][0]*(d_x - x_2[at_id_r2[k]][at_id_r[k]]);
                           dL_y_r[i]     += atom_masses[i]*state_global->v[at_id[i]][1]*(d_y - y_2[at_id_r2[k]][at_id_r[k]]);
                           dL_z_r[i]     += atom_masses[i]*state_global->v[at_id[i]][2]*(d_z - z_2[at_id_r2[k]][at_id_r[k]]);                             
      ...
      if(i == at_id_r[k] || i == at_id_r2[k]) {                
      
        diff_s_x1_r[i] = dL_x_r[i]; 
        diff_s_y1_r[i] = dL_y_r[i];
        diff_s_z1_r[i] = dL_z_r[i];
        
        diff_s_x_r[i] += dL_x_r[i];
        diff_s_y_r[i] += dL_y_r[i];
        diff_s_z_r[i] += dL_z_r[i];
        
       if(diff_s_x1_r[i] - diff_s_x_r[i]/tau_1[i] != 0.0 && diff_s_x2_r[i] - diff_s_x_r[i]/tau_1[i] != 0.0) {
           
          diff_s_xc_r[i] += (diff_s_x2_r[i] - diff_s_x_r[i]/tau_1[i])*(diff_s_x1_r[i] - diff_s_x_r[i]/tau_1[i])/
          (sqrt(pow(diff_s_x2_r[i] - diff_s_x_r[i]/tau_1[i],2))*sqrt(pow(diff_s_x1_r[i] - diff_s_x_r[i]/tau_1[i],2)));
          dLL_x_r[i]       += (diff_s_x2_r[i] - diff_s_x_r[i]/tau_1[i])*(diff_s_x1_r[i] - diff_s_x_r[i]/tau_1[i])/
          (sqrt(pow(diff_s_x2_r[i] - diff_s_x_r[i]/tau_1[i],2))*sqrt(pow(diff_s_x1_r[i] - diff_s_x_r[i]/tau_1[i],2))); 
          
          diff_L_xc[i]     += (diff_s_x2_r[i] - diff_s_x_r[i]/tau_1[i])*(diff_s_x1_r[i] - diff_s_x_r[i]/tau_1[i])/
          (sqrt(pow(diff_s_x2_r[i] - diff_s_x_r[i]/tau_1[i],2))*sqrt(pow(diff_s_x1_r[i] - diff_s_x_r[i]/tau_1[i],2))); 
          
          counter_restraint_x[i] += 1.0;

       };
       if(diff_s_y1_r[i] - diff_s_y_r[i]/tau_1[i] != 0.0 && diff_s_y2_r[i] - diff_s_y_r[i]/tau_1[i] != 0.0) {
           
          diff_s_yc_r[i] += (diff_s_y2_r[i] - diff_s_y_r[i]/tau_1[i])*(diff_s_y1_r[i] - diff_s_y_r[i]/tau_1[i])/
          (sqrt(pow(diff_s_y2_r[i] - diff_s_y_r[i]/tau_1[i],2))*sqrt(pow(diff_s_y1_r[i] - diff_s_y_r[i]/tau_1[i],2)));
          dLL_y_r[i]       += (diff_s_y2_r[i] - diff_s_y_r[i]/tau_1[i])*(diff_s_y1_r[i] - diff_s_y_r[i]/tau_1[i])/
          (sqrt(pow(diff_s_y2_r[i] - diff_s_y_r[i]/tau_1[i],2))*sqrt(pow(diff_s_y1_r[i] - diff_s_y_r[i]/tau_1[i],2)));
          
          diff_L_yc[i]    += (diff_s_y2_r[i] - diff_s_y_r[i]/tau_1[i])*(diff_s_y1_r[i] - diff_s_y_r[i]/tau_1[i])/
          (sqrt(pow(diff_s_y2_r[i] - diff_s_y_r[i]/tau_1[i],2))*sqrt(pow(diff_s_y1_r[i] - diff_s_y_r[i]/tau_1[i],2)));
          
          counter_restraint_y[i] += 1.0;

       };
          
       if(diff_s_z1_r[i] - diff_s_z_r[i]/tau_1[i] != 0.0 && diff_s_z2_r[i] - diff_s_z_r[i]/tau_1[i] != 0.0) {
           
          diff_s_zc_r[i] += (diff_s_z2_r[i] - diff_s_z_r[i]/tau_1[i])*(diff_s_z1_r[i] - diff_s_z_r[i]/tau_1[i])/
          (sqrt(pow(diff_s_z2_r[i] - diff_s_z_r[i]/tau_1[i],2))*sqrt(pow(diff_s_z1_r[i] - diff_s_z_r[i]/tau_1[i],2)));
          dLL_z_r[i]       += (diff_s_z2_r[i] - diff_s_z_r[i]/tau_1[i])*(diff_s_z1_r[i] - diff_s_z_r[i]/tau_1[i])/
          (sqrt(pow(diff_s_z2_r[i] - diff_s_z_r[i]/tau_1[i],2))*sqrt(pow(diff_s_z1_r[i] - diff_s_z_r[i]/tau_1[i],2)));
          
          diff_L_zc[i]     += (diff_s_z2_r[i] - diff_s_z_r[i]/tau_1[i])*(diff_s_z1_r[i] - diff_s_z_r[i]/tau_1[i])/
          (sqrt(pow(diff_s_z2_r[i] - diff_s_z_r[i]/tau_1[i],2))*sqrt(pow(diff_s_z1_r[i] - diff_s_z_r[i]/tau_1[i],2)));
         
	  counter_restraint_z[i] += 1.0;

       };
          
       ran = rand()%100/100.0;
    
       if(ran <= 1.0/(1.0 + exp(-((diff_s_xc_r[i]/tau_1[i]))))) {
      
         diff_s_x2_r[i] = diff_s_x1_r[i];
            
        };
    
        if(ran <= 1.0/(1.0 + exp(-((diff_s_yc_r[i]/tau_1[i]))))) {    
        
          diff_s_y2_r[i] = diff_s_y1_r[i];         
             
         };
    
         if(ran <= 1.0/(1.0 + exp(-((diff_s_zc_r[i]/tau_1[i]))))) {    
        
          diff_s_z2_r[i] = diff_s_z1_r[i];  
     ...
     if(i == at_id_r[k]) {    
        ...
        x_2[at_id_r[k]][at_id_r2[k]] = d_x;
        y_2[at_id_r[k]][at_id_r2[k]] = d_y;      
        z_2[at_id_r[k]][at_id_r2[k]] = d_z;
      
       };
       
     if(i == at_id_r2[k]) {    
       
        x_2[at_id_r2[k]][at_id_r[k]] = d_x;
        y_2[at_id_r2[k]][at_id_r[k]] = d_y;      
        z_2[at_id_r2[k]][at_id_r[k]] = d_z;
...
double d_x3,d_y3,d_z3,range_x,range_y,range_z;
   ...
   for(i=1;i<=natoms1;i++) {
       ...
       if(restraint_sampling[i] == 1 && counter_restraint_x[i] != 0.0) {       
         
           bias_x[i] = (diff_s_xc[i]/tau_1[i])*(diff_s_xc_r[i]/(tau_1[i]*counter_restraint_x[i]));

        };

       if(restraint_sampling[i] == 1 && counter_restraint_y[i] != 0.0) {

	      bias_y[i] = (diff_s_yc[i]/tau_1[i])*(diff_s_yc_r[i]/(tau_1[i]*counter_restraint_y[i]));

       };

       if(restraint_sampling[i] == 1 && counter_restraint_z[i] != 0.0) {

	      bias_z[i] = (diff_s_zc[i]/tau_1[i])*(diff_s_zc_r[i]/(tau_1[i]*counter_restraint_z[i]));
   ...
   if(stepper%1000 == 0.0) {   
       ...
       for(k=1;k<=100;k++) {
           ...
           hist_global_x[i][k] += exp(-pow(bias_x[i] - range_x,2)/0.02);
           hist_global_y[i][k] += exp(-pow(bias_y[i] - range_y,2)/0.02);
           hist_global_z[i][k] += exp(-pow(bias_z[i] - range_z,2)/0.02);
           
           weight_x[k]         += exp(-pow(bias_x[i] - range_x,2)/0.02);
           weight_y[k]         += exp(-pow(bias_y[i] - range_y,2)/0.02);
           weight_z[k]         += exp(-pow(bias_z[i] - range_z,2)/0.02);
...
for(i=1;i<=natoms1;i++) {   
   ...
   meas_x[i] = (int)((bias_x[i] + 1)/0.02);
   meas_y[i] = (int)((bias_y[i] + 1)/0.02);
   meas_z[i] = (int)((bias_z[i] + 1)/0.02);         
       
    if(meas_x[i] != 0 && meas_y[i] != 0 && meas_z[i] != 0 && meas_x[i] <= 100 && meas_y[i] <= 100 && meas_z[i] <= 100 && stepper > 1000) {  
        
   //printf("%lg\t%lg\t%lg\n",alpha_2_x[i],alpha_2_y[i],alpha_2_z[i]);
   //printf("%lg\t%lg\t%lg\n",beta_2_x[i],beta_2_y[i],beta_2_z[i]);             
        
        if(weight_x[meas_x[i]] != 0.0 && weight_x[meas_x[i]-1] && hist_global_x[i][meas_x[i]] != 0.0 && hist_global_x[i][meas_x[i]-1] != 0.0) 
            d_x  = (log(hist_global_x[i][meas_x[i]]/(weight_x[meas_x[i]]))-log(hist_global_x[i][meas_x[i]-1]/(weight_x[meas_x[i]-1])))*100.0;

        if(weight_y[meas_y[i]] != 0.0 && weight_y[meas_y[i]-1] && hist_global_y[i][meas_y[i]] != 0.0 && hist_global_y[i][meas_y[i]-1] != 0.0) 
            d_y  = (log(hist_global_y[i][meas_y[i]]/(weight_y[meas_y[i]]))-log(hist_global_y[i][meas_y[i]-1]/(weight_y[meas_y[i]-1])))*100.0;  
        
        if(weight_z[meas_x[i]] != 0.0 && weight_z[meas_z[i]-1] && hist_global_z[i][meas_z[i]] != 0.0 && hist_global_z[i][meas_z[i]-1] != 0.0) 
            d_z  = (log(hist_global_z[i][meas_z[i]]/(weight_z[meas_z[i]]))-log(hist_global_z[i][meas_z[i]-1]/(weight_z[meas_z[i]-1])))*100.0;
        
          f_global[at_id[i]][0] = f_global[at_id[i]][0]*exp(-bias_x[i])*(1.0 + bias_x[i]) - alpha*d_x*exp(-bias_x[i])*(1.0 + bias_x[i]);    
          f_global[at_id[i]][1] = f_global[at_id[i]][1]*exp(-bias_y[i])*(1.0 + bias_y[i]) - alpha*d_y*exp(-bias_y[i])*(1.0 + bias_y[i]);   
          f_global[at_id[i]][2] = f_global[at_id[i]][2]*exp(-bias_z[i])*(1.0 + bias_z[i]) - alpha*d_z*exp(-bias_z[i])*(1.0 + bias_z[i]);         
